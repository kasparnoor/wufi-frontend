...
export async function GET() {
  ...
  const res = await fetch(`${BASE}/store/customers/me/notifications`, { headers, cache: "no-store" })
...
export async function PATCH(req: NextRequest) {
  ...
  const res = await fetch(`${BASE}/store/customers/me/notifications`, { method: "PATCH", headers, body: JSON.stringify(body), cache: "no-store" })
  const data = await res.json().catch(() => ({}))
  return NextResponse.json(data, { status: res.status })
}
