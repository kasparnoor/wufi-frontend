...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/store/customers/me/subscriptions/${params.id}`, { headers, cache: "no-store" })
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await fetch(`${BASE}/store/customers/me/subscriptions/${params.id}`, { method: "PATCH", headers, body: JSON.stringify(body), cache: "no-store" })
  const data = await res.json().catch(() => ({}))
  return NextResponse.json(data, { status: res.status })
}
